...
Ext.define('Ext.d3.HeatMap', {
    ...
    performLayout: function(rect) {
        ...
        var legend = me.getLegend(),
            ...
            xScale = xAxis.getScale(),
            yScale = yAxis.getScale(),
            tilesRect, legendRect, titleRect, shrinkRect;
        shrinkRect = {
            x: 0,
            y: 0,
            width: rect.width,
    ...
    onUpdateTiles: function(selection) {
        ...
        selection.select('rect').attr('x', function(item) {
            ...
            return yScale(value);
        }).attr('width', xBand || xScale(xStep) - xScale(0)).attr('height', yBand || yScale(0) - yScale(yStep)).style('fill', function(item) {
...
/**
 * The base class of every D3 Component that can also be used standalone.
 * For example:
 *
 *     @example
 *     Ext.create({
 ...
 *
 *         listeners: {
 *             canvasresize: function (component, canvas, size) {
 *                 var barCount = 10,
...
Ext.define('Ext.d3.canvas.Canvas', {
    ...
    config: {
         ...
         * This can also be useful when this class is used in conjunction with
         * another Canvas library that provides HDPI support as well.
         * Once set cannot be changed.
         * @cfg {Boolean} [hdpiSupport=true]
         */
        hdpiSupport: true
    },
    requires: [
...
Ext.define('Ext.d3.hierarchy.Hierarchy', {
     ...
     * @param {Number} index
     */
    addNodeListeners: function(element, node) {
        var me = this,
            el = Ext.get(element),
            selectEventName = me.getSelectEventName(),
            expandEventName = me.getExpandEventName();
        selectEventName && el.on(selectEventName, me.onSelectEvent, me);
        expandEventName && el.on(expandEventName, me.onExpandEvent, me);
        el.on('mousemove', function(event) {
            me.onNodeMouseMove(event, node, element);
        });
    },
    ...
    onNodeMouseMove: function(event, node, element) {
        if (!this.isMouseDown) {
            this.showTooltip(event, node, element);
        }
    },
    tooltipTimeout: null,
    showTooltip: function(event, node, element) {
        var me = this,
        ...
        // tooltip.currentTarget.attach((node.sprite.length ? node.sprite[0] : node.sprite).getSurface().el.dom);
...
Ext.define('Ext.d3.hierarchy.Pack', {
    ...
    textVisibilityFn: function(selection) {
        ...
        selection.attr('visibility', function(node) {
    ...
    updateNodes: function(selection) {
        ...
        selection.selectAll('text').transition().call(me.textVisibilityFn.bind(me));
    }
});
...
Ext.define('Ext.d3.hierarchy.TreeMap', {
    ...
    applyLayout: function() {
        var me = this,
            sticky = me.getSticky(),
            layout = d3.layout.treemap().round(false).sticky(sticky);
        return layout;
    },
    setLayoutSize: function(size) {
        this.callParent([
...
Ext.define('Ext.d3.hierarchy.partition.Sunburst', {
    ...
    textVisibilityFn: function(selection) {
        ...
        // click and other events on adjacent node elements.
        selection[method]('visibility', visibilityFn)[method]('fill-opacity', fillOpacityFn);
    },
    /**
     * @private
...
/**
 * Defines a common abstract parent class for all D3 behaviors.
 */
Ext.define('Ext.d3.interaction.Abstract', {
    ...
    config: {
         ...
         * touch-action to apply to the {@link #component D3 Component}
         * when this interaction is in use.
         *
         * See {@link Ext.Widget#touchAction} for more details.
         */
        touchAction: null
    },
    /**
    ...
    updateComponent: function(component, oldComponent) {
        ...
        if (component) {
            ...
            if (touchAction) {
                component.setTouchAction(touchAction);
            }
        }
    },
...
Ext.define('Ext.d3.interaction.PanZoom', {
    ...
    config: {
        pan: {
            gesture: 'drag',
            // If `false`, the panning will be unconstrained, even if `contentRect` and
            // `viewportRect` configs are set.
            constrain: true,
            momentum: {
                // The magnitude of the friction force.
                friction: 1,
                // Spring force will be proportional to the degree of bounds penetration
                // (displacement from equilibrium position), as well as this spring constant.
    ...
    toViewportXY: function(pageX, pageY) {
        var elementXY = this.component.element.getXY(),
            ...
            result = [
                pageX - elementXY[0] - (viewportRect ? viewportRect.x : 0),
                pageY - elementXY[1] - (viewportRect ? viewportRect.y : 0)
            ];
        return result;
    },
    /**
    ...
    onPanGestureStart: function(e) {
        ...
        if (pan && (!touches || touches.length < 2)) {
            ...
            xy = e.getXY();
            me.panOrigin = me.lastPanXY = me.toViewportXY(xy[0], xy[1]);
